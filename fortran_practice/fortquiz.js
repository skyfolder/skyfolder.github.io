...
(function (root) {
  ...
  function renderGate(err) {
    ...
    function go() {
      var n = nameEl.value.trim(), s = sidEl.value.trim();
  ...
  function fileName() {
    /* 이름_챕터_점수.txt */
    return safeName(student.name) + '_' + CFG.chapter + '_' + score() + '.txt';
  }
  ...
  function downloadReport() {
    ...
    savedOnce = true;
    saveMsg(fileName() + ' 을(를) 내려받았습니다. 다운로드 폴더에서 꺼내 제출하세요.', 'ok');
  }
    ...
    window.showDirectoryPicker({ mode: 'readwrite' })
      .then(function (dir) {
        return dir.getFileHandle(fileName(), { create: true })
          ...
          .then(function (w) { return w.write('\uFEFF' + buildReport()).then(function () { return w.close(); }); })
          .then(function () {
            savedOnce = true;
            saveMsg(dir.name + ' 폴더에 ' + fileName() + ' 을(를) 저장했습니다.', 'ok');
          });
      })
      .catch(function (e) {
  ...
  function renderReport() {
    ...
    app.innerHTML =
      ...
      '<div class="report">' +
        ...
        '</div>' +
        '<div id="save-msg"></div>' +
        '<span class="box-label">저장될 파일 · ' + esc(fileName()) + '</span>' +
        '<pre class="preview">' + esc(buildReport()) + '</pre>' +
      '</div>';
  }
